---
Date: 2025-11-25 09:35:48
LastEditTime: 2025-11-25 11:27:41
aside: false
description: 科研网站/资源推荐，其中不仅有Qbics、Qbics-Molstar等还有其他的优质网站/资源。
tags:
  - sci
  - site
  - recs
  - 科研
  - 推荐
  - qbics
  - molstar
head:
  - - meta
    - name: keywords
      content: molstar, 科研推荐, 科研网站推荐
---

# 科研网站/资源推荐

<script lang="ts" setup>
import Features from '../../components/Features';
import qbics from './assets/qbics.webp';
import qbicsMolstar from './assets/qbics-molstar.svg';

const features = [
  {
    icon: {
      src: qbics,
      alt: 'qbics',
    },
    title: 'Qbics',
    details: 'Qbics 是深圳湾实验室开发的一款独具特色的计算化学软件，是的新一代电子结构软件，其开发重点是利用密度泛函理论和QM/MM方法进行复杂体系的能量和动力学模拟。',
    link: 'https://zhjun-sci.com/qbics.html',
    linkText: '跳转官网'
  },
  {
    icon: {
      src: qbicsMolstar,
      alt: 'Qbics Molstar',
    },
    title: 'Qbics-MolStar',
    details: 'Qbics 团队在开源软件 MolStar 基础上自主开发的 Qbics-MolStar 可视化工具，大大提高了研究工作的效率。Qbics-MolStar支持计算化学中常用的所有坐标、轨迹、波函数格式、SMILES字符串或PubChem数据库直接导入分子结构、分子动力学轨迹、波函数可视化。<a herf="https://molstar.szbl.ac.cn/download/" target="_blank">【客户端下载】</a>',
    link: 'https://molstar.szbl.ac.cn/viewer/',
    linkText: '跳转官网'
  }
]
</script>

<Features :features/>


## 文献检索常用网站

1. 综合数据库

    - SciFinder  http://scifinder.cas.org
    - Reaxy http://www.reaxys.com/
    - Web of Science https://www.webofscience.com
    - Pubmed http://www.ncbi.nlm.nih.gov

2. 出版社数据库

    - ScienceDirect http://www.sciencedirect.comACS
    - Publications http://pubs.acs.org
    - Springer http://link.springer.com
    - Wiley http://onlinelibrary.wiley.comRSC
    - Publishing http://pubs.rsc.org

3. 学术搜索引擎

    - 百度学术 https://xeshu.baidu.com/
    - 必应学术 https://cn.bing.com/academic
    - 谷歌学术 https://scholar.google.com/

4. 中文数据库

    - 知网 https://www.cnki.net/
    - 万方 https://www.wanfangdata.com.cn/
    - 维普 http://www.cqvip.com/

5. 文献下载

    - SCI-HUB https://sci-hub.se/

## 专利检索常用网站

- 中国国家知识产权局 https://www.cnipa.gov.cn/
- 欧洲专利局 https://worldwide.espacenet.com/
- 世界知识产权组织 https://patentscope2.wipo.int/
- 美国专利商标局 https://www.uspto.gov/
- 药物在线 http://www.drugfuture.com/
- 中国专利信息中心 http://www.cnpat.com.cn/
- 谷歌专利 https://patents.google.com/
- FPO http://www.freepatentsonline.com/
- 智慧芽 https://account.zhihuiya.com/