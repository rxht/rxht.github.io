...
import { FormatDate } from "./date.mts";
...
export default createContentLoader("/**/*.md", {
  includeSrc: true,
  transform(rawPosts) {
    const config = global.VITEPRESS_CONFIG as {
      site: { base: string };
      srcDir: string;
    };
    if (!config) {
...
function normalizeSort(a: ContentData, b: ContentData): number {
  ...
  if (aLastEditTime && bLastEditTime) {
    return (
      new Date(bLastEditTime).getTime() - new Date(aLastEditTime).getTime()
    );
  }
...
function normalizeTitle(frontmatter: Record<string, any>, fileContent: string) {
  ...
  const title = (lines.find((item) => item.startsWith("#")) ?? "")
    .replace("#", "")
    .trim();
  frontmatter.title = title;
}
function normalizeLastEditTime(
